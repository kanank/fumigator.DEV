...
type
  ...
  public
    ...
    property HttpContentType: THttpContentType read fHttpContentType write fHttpContentType; //дл€ Post
  ...
  private
    ...
    fTimer: TTimer;
    fAutoReGet: Boolean;
    fTimeExpires: TDateTime; //врем€ истечени€
  ...
  private
    ...
    fOnAfterChange: TNotifyEvent; // после изменени€ звонка
    fCallId: string;
    fExtension: string;
  ...
  public
    property Active: Boolean read fActive write SetActive; //перевод в активное состо€ние
 ...
 // stream.WriteString('state=0');
  ...
  try
    ...
    try
      ...
      Result := false;
      fToken := '';
      fResponseContent := 'ќшибка: ' + Self.HttpErr + #13#10 +
